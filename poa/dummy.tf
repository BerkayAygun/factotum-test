resource "openstack_compute_instance_v2" "dummy" {
	count = "1"
	name = "pr-dummy-po-a-00${count.index + 1}"
	region = "PO-A"
	image_id = "9da95274-3b23-49b0-a341-fd72b5afd3a7"
	flavor_id = "3104"
	key_pair = "pcmm-production"
	security_groups = ["default","mongo_access"]
...
resource "vinyldns_record_set" "dummy_record_set_v4" {
	count = "1"
	name = "pr-dummy-po-a-00${count.index + 1}"
	zone_id = "1bbdd4b5-1034-4578-8c19-8ed70ffe083c"
	type = "A"
	ttl = 300
	record_addresses = ["${element(openstack_compute_instance_v2.dummy.*.access_ip_v4, count.index)}"]
}
